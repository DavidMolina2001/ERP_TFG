...
#Region "Informe Envios"
    ...
    Private Sub GenerarInformeEnvios()
        ...
        Try
            ...
            Dim sOutput As String = clsConstants.PathGuardadoInformes & "\InformeEnvios-" & Date.Now.ToString("dd-MM-yyyy") & "-" & Date.Now.Ticks & ".xlsx"

            oBook.Save(sOutput, True)

            GuardarDocumento(sOutput, 0)

            frmWaitForm.CloseWaitForm()


            If frmMissatge.MostrarPreguntaSiNo(fnTraductor.RetornaIdiomaSeleccionat("Informe generat correctament, el vols obrir?",
                                                                         "Informe generado correctamente, ¿lo quieres abrir?")) Then

                Process.Start("C:\Program Files\Microsoft Office\root\Office16\excel.EXE", sOutput)
...
#End Region
    ...
    Private Sub GenerarInformeFacturacio()
        ...
        Try
            ...
            Dim sOutput As String = clsConstants.PathGuardadoInformes & "\InformeFactures-" & Date.Now.ToString("dd-MM-yyyy") & "-" & Date.Now.Ticks & ".xlsx"

            oBook.Save(sOutput, True)

            GuardarDocumento(sOutput, 1)
            ...
            If frmMissatge.MostrarPreguntaSiNo(fnTraductor.RetornaIdiomaSeleccionat("Informe generat correctament, el vols obrir?",
                ...
                'Process.Start("excel.exe", sOutput)
                Process.Start("C:\Program Files\Microsoft Office\root\Office16\excel.EXE", sOutput)
...
#End Region
    ...
    Public Shared Function GuardarDocumento(sRuta As String, iTipusDocumento As Byte)
        Dim oDocumento As clsDocuments = Nothing
        Dim iId As Integer = 0
        Try
            Dim nombreArchivo As String = Path.GetFileName(sRuta)

            oDocumento = New clsDocuments()
            oDocumento.NomDocument = nombreArchivo
            oDocumento.RutaDocument = sRuta
            oDocumento.TipusDocument = iTipusDocumento

            oDocumento.Escriure(True)

            iId = oDocumento.Id

        Catch ex As Exception
            Throw New Exception("Error GuardarDocumento", ex)
        End Try
        Return iId
    End Function
